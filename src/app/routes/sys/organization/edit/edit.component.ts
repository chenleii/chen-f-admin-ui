...
@Component({
  selector: 'app-sys-organization-edit',
...
export class SysOrganizationEditComponent implements OnInit {
  ...
  ngOnInit(): void {
    zip(
      ...
      this.cacheService.get('/chen/common/sys/dictionary/item/alain/select/STATUS',
        { mode: 'promise', type: 's', expire: 86400 }),
    ).subscribe(([sysOrganization, allSysOrganization, typeSelect, statusSelect]: any) => {
  ...
  save(value: any) {
    this.http.post(`/chen/admin/sys/organization/${this.record.id}`, value).subscribe(res => {
